...
func NewHyperFleetHandler(cfg *LogConfig) slog.Handler {
	// For text format, use HyperFleet-specific text handler
	if cfg.Format == FormatText {
		return NewHyperFleetTextHandler(cfg.Output, cfg.Component, cfg.Version, cfg.Hostname, cfg.Level)
	}

	// For JSON format, use standard slog JSON handler with custom attributes
...
func (h *HyperFleetHandler) Handle(ctx context.Context, r slog.Record) error {
	...
	// Extract string-type context fields using registry
	...
	// Extract transaction_id separately (int64 type, DB operations only)
...
// For temporary fields, use With(): logger.With(ctx, "key", value).Debug("message")
func Debug(ctx context.Context, msg string) {
	GetLogger().DebugContext(ctx, msg)
}

// Info logs at Info level with context fields only.
// For temporary fields, use With(): logger.With(ctx, "key", value).Info("message")
func Info(ctx context.Context, msg string) {
	GetLogger().InfoContext(ctx, msg)
}

// Warn logs at Warn level with context fields only.
// For temporary fields, use With(): logger.With(ctx, "key", value).Warn("message")
func Warn(ctx context.Context, msg string) {
	GetLogger().WarnContext(ctx, msg)
}

// Error logs at Error level with context fields only.
// For temporary fields, use With(): logger.With(ctx, "key", value).Error("message")
...
// Context fields (persistent): request_id, trace_id, span_id, cluster_id, resource_type, resource_id, transaction_id
// Temporary fields: Any key-value pairs passed to With()
...
// Example: logger.With(ctx, "user_id", userID, "action", "login").Info("User logged in")
...
func (l *ContextLogger) Debug(msg string) {
	GetLogger().DebugContext(l.ctx, msg, l.args...)
}

func (l *ContextLogger) Info(msg string) {
	GetLogger().InfoContext(l.ctx, msg, l.args...)
}

func (l *ContextLogger) Warn(msg string) {
	GetLogger().WarnContext(l.ctx, msg, l.args...)
}

func (l *ContextLogger) Error(msg string) {
	GetLogger().ErrorContext(l.ctx, msg, l.args...)
}
