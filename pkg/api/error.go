...
func SendNotFound(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		logger.With(r.Context(), "error", err).Error("Failed to send response")
		return
	}
}
...
func SendPanic(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		logger.With(r.Context(), "error", err).Error("Failed to send panic response")
	}
}
...
func init() {
	...
	if err != nil {
		...
		logger.With(ctx, "error", err).Error("Failed to create panic error body")
		os.Exit(1)
	}
}
