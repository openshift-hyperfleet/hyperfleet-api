...
func TestPresentNodePool_Complete(t *testing.T) {
	...
	Expect(result.Status.Conditions[0].Type).To(Equal(api.AdapterConditionTypeAvailable))
...
func TestPresentNodePool_StatusConditionsConversion(t *testing.T) {
	...
	// Second condition
	Expect(result.Status.Conditions[1].Type).To(Equal(api.AdapterConditionTypeHealth))
