...
func (m *MaskingMiddleware) MaskBody(body []byte) []byte {
	...
	if err := json.Unmarshal(body, &data); err != nil {
		...
		logger.With(context.Background(),
			"error", err.Error()).Warn("JSON parsing failed in MaskBody, applying text-based fallback masking")
		return m.maskTextFallback(body)
	}
	...
	if err != nil {
		...
		logger.With(context.Background(),
			"error", err.Error()).Warn("JSON marshaling failed in MaskBody, applying text-based fallback masking")
		return m.maskTextFallback(body)
	}
	return masked
}
