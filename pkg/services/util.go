...
func handleRequestDeletionError(resourceType string, err error) *errors.ServiceError {
	if e.Is(err, gorm.ErrRecordNotFound) {
	...
	return errors.GeneralError("Unable to request deletion of %s: %s", resourceType, err.Error())
}
