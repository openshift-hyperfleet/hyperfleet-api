...
func AggregateResourceStatus(in AggregateResourceStatusInput) (
	ready, available api.ResourceCondition, adapterConditions []api.ResourceCondition,
) {
	prevReady, prevAvail, prevAdapterByType := parsePrevConditions(in.PrevConditionsJSON)

	reports := normalizeAdapterReportsForAggregation(in.AdapterStatuses, in.RequiredAdapters, in.ResourceGeneration)
...
func parsePrevConditions(raw []byte) (
...
func computeReady(
	...
	lastUpdated := computeReadyLastUpdatedTime(
		resourceGen, refTime, required, byAdapter, status,
	)
	...
	reason := "AdaptersNotAtCurrentGeneration"
	message := "Adapters have not all reported Available=True at the current generation"
...
func computeReadyLastUpdatedTime(
	...
	required []string,
	byAdapter map[string]adapterAvailableSnapshot,
	readyStatus api.ResourceConditionStatus,
) time.Time {
	if readyStatus != api.ConditionTrue {
		return refTime
	}
	...
	// Ready=True implies every required adapter is at the current generation — use the oldest observation.
...
func computeAvailable(
	...
	if len(required) > 0 && !missingReport {
		if allTrue {
			if !mixed {
				status = api.ConditionTrue
			} else if prev != nil && prev.Status == api.ConditionTrue {
				// All adapters True but at different generations — keep True only if aggregate was already True.
				status = api.ConditionTrue
			}
		} else if prev != nil && prev.Status == api.ConditionTrue {
			// Some adapter reports Available=False — keep True unless the failure is at the tracked generation.
			tracked := prev.ObservedGeneration
			falseAtTracked := false
			for _, name := range required {
				s := byAdapter[name]
