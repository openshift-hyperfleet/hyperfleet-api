...
func (d *sqlClusterDao) RequestDeletion(ctx context.Context, id string) (*api.Cluster, bool, error) {
	...
	// Set deleted_at and increment generation to trigger Sentinel reconciliation.
	t := time.Now()
	cluster.DeletedAt = &t
	cluster.Generation++
	if err := g2.Omit(clause.Associations).Save(cluster).Error; err != nil {
...
// Phase 1 (pending deletion) is handled by RequestDeletion, which sets deleted_at via an
...
// approach (current) or switch to GORM's native soft-delete by changing Meta.DeletedAt back
// to gorm.DeletedAt. The GORM approach would auto-filter queries but requires Unscoped() for
