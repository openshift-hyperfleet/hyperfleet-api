...
func Resolve(ctx context.Context) {
	...
	if tx.MarkedForRollback() {
		if err := tx.Rollback(); err != nil {
			logger.Error(ctx, "Could not rollback transaction", "error", err.Error())
			return
		}
		...
		if err := tx.Commit(); err != nil {
			...
			logger.Error(ctx, "Could not commit transaction", "error", err.Error())
			return
		}
	}
}
...
func MarkForRollback(ctx context.Context, err error) {
	...
	logger.Info(ctx, "Marked transaction for rollback", "error", err)
}
