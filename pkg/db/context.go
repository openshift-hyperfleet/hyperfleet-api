...
func Resolve(ctx context.Context) {
	...
	if tx.MarkedForRollback() {
		if err := tx.Rollback(); err != nil {
			logger.With(ctx, "error", err.Error()).Error("Could not rollback transaction")
			return
		}
		...
		if err := tx.Commit(); err != nil {
			...
			logger.With(ctx, "error", err.Error()).Error("Could not commit transaction")
			return
		}
	}
}
...
func MarkForRollback(ctx context.Context, err error) {
	...
	logger.With(ctx, "error", err).Info("Marked transaction for rollback")
}
