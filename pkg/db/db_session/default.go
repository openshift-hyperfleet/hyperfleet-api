...
func waitForNotification(l *pq.Listener, callback func(id string)) {
	...
	for {
		select {
		case n := <-l.Notify:
			logger.With(ctx, "channel", n.Channel, "data", n.Extra).Info("Received data from channel")
			callback(n.Extra)
			return
		case <-time.After(10 * time.Second):
			...
			go func() {
				if err := l.Ping(); err != nil {
					logger.With(ctx, "error", err).Debug("Ping failed")
				}
			}()
			return
		}
...
func newListener(ctx context.Context, connstr, channel string, callback func(id string)) {
	...
	logger.With(ctx, "channel", channel).Info("Starting channeling monitor")
	for {
