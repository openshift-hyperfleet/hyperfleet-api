...
func (f *Testcontainer) Init(config *config.DatabaseConfig) {
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Failed to start PostgreSQL testcontainer")
		os.Exit(1)
	}
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Failed to get connection string from testcontainer")
		os.Exit(1)
	}

	logger.With(ctx, "connection_string", redactPassword(connStr)).Info("PostgreSQL testcontainer started")

	// Open SQL connection
	f.sqlDB, err = sql.Open("postgres", connStr)
	if err != nil {
		logger.With(ctx, "error", err).Error("Failed to connect to testcontainer database")
		os.Exit(1)
	}
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Failed to connect GORM to testcontainer database")
		os.Exit(1)
	}
	...
	if err := db.Migrate(f.g2); err != nil {
		logger.With(ctx, "error", err).Error("Failed to run migrations on testcontainer")
		os.Exit(1)
	}
...
func (f *Testcontainer) Close() error {
	...
	if f.sqlDB != nil {
		if err := f.sqlDB.Close(); err != nil {
			logger.With(ctx, "error", err).Error("Error closing SQL connection")
		}
	}
	...
	for _, table := range tables {
		if g2.Migrator().HasTable(table) {
			if err := g2.Exec(fmt.Sprintf("TRUNCATE TABLE %s CASCADE", table)).Error; err != nil {
				logger.With(ctx, "table", table, "error", err).Error("Error truncating table")
			}
		}
	}
}
...
func (f *Testcontainer) NewListener(ctx context.Context, channel string, callback func(id string)) {
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Failed to get connection string for listener")
		return
	}
