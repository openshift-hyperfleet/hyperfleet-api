...
func TestAsProblemDetails(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "no validation details means nil errors array",
			build:          func() *ServiceError { return Validation("bad input") },
			expectedType:   ErrorTypeValidation,
		...
		},
		{
			...
			build: func() *ServiceError {
				return ValidationWithDetails("validation failed", []ValidationDetail{
					...
					{Field: "size", Value: 0, Message: "must be positive", Constraint: "min"},
		...
		{
			name: "validation details with empty slice produces nil errors",
			build: func() *ServiceError {
				return ValidationWithDetails("validation failed", []ValidationDetail{})
			},
			expectedType:   ErrorTypeValidation,
		...
		},
		{
			...
			build: func() *ServiceError {
				return ValidationWithDetails("validation failed", []ValidationDetail{
					{Field: "foo", Message: "bad value", Constraint: ""},
				})
			},
			expectedType:   ErrorTypeValidation,
			expectedTitle:  "Validation Failed",
