...
func NewHelper(t *testing.T) *Helper {
	once.Do(func() {
		...
		if err != nil {
			logger.With(ctx, "error", err).Error("Unable to add environment flags")
			os.Exit(1)
		}
		if logLevel := os.Getenv("LOGLEVEL"); logLevel != "" {
			logger.With(ctx, "level", logLevel).Info("Using custom loglevel")
		...
		err = env.Initialize()
		if err != nil {
			logger.With(ctx, "error", err).Error("Unable to initialize testing environment")
			os.Exit(1)
		}
...
func (helper *Helper) startAPIServer() {
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Unable to start Test API server")
		os.Exit(1)
	}
	go func() {
...
func (helper *Helper) stopMetricsServer() {
	...
	if err := helper.MetricsServer.Stop(); err != nil {
		logger.With(ctx, "error", err).Error("Unable to stop metrics server")
		os.Exit(1)
	}
}
...
func (helper *Helper) RestartServer() {
	...
	if err := helper.stopAPIServer(); err != nil {
		logger.With(ctx, "error", err).Warn("unable to stop api server on restart")
	}
	helper.startAPIServer()
...
func (helper *Helper) Reset() {
	...
	if err := env.AddFlags(flagset); err != nil {
		logger.With(ctx, "error", err).Error("Unable to add environment flags on Reset")
		os.Exit(1)
	}
	pflag.Parse()
	...
	err := env.Initialize()
	if err != nil {
		logger.With(ctx, "error", err).Error("Unable to reset testing environment")
		os.Exit(1)
	}
