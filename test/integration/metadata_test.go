...
func TestMetadataGet(t *testing.T) {
	...
	Expect(metadata.ID).To(Equal("hyperfleet"), "Expected ID to be 'hyperfleet'")
	Expect(metadata.Kind).To(Equal("API"), "Expected Kind to be 'API'")
	Expect(metadata.HREF).To(Equal("/api/hyperfleet"), "Expected HREF to match the request path")
