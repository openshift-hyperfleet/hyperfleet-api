...
func TestPendingDeletionCollector_Integration(t *testing.T) {
	...
	t.Run("given soft-deleted resources within threshold, collector reports zero stuck", func(t *testing.T) {
		...
		clusterValue, ok := collected["cluster"]
		Expect(ok).To(BeTrue(), "collector should emit a cluster series")
		Expect(clusterValue).To(Equal(0.0), "recently deleted cluster should not be stuck")
	})
}
