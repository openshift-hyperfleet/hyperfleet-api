...
func TestMain(m *testing.M) {
	flag.Parse()
	slog.Info("Starting integration test", "go_version", runtime.Version())
	...
	if os.Getenv("OPENAPI_SCHEMA_PATH") == "" {
		...
		if !ok {
			slog.Warn("Failed to determine current file path via runtime.Caller, skipping OPENAPI_SCHEMA_PATH setup")
		} else {
			...
			if _, err := os.Stat(schemaPath); err != nil {
				slog.Warn("Schema file not found, skipping OPENAPI_SCHEMA_PATH setup", "path", schemaPath, "error", err)
			} else {
				_ = os.Setenv("OPENAPI_SCHEMA_PATH", schemaPath)
				slog.Info("Set OPENAPI_SCHEMA_PATH for integration tests", "path", schemaPath)
			}
		}
	}
