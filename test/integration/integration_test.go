	...
	if os.Getenv("OPENAPI_SCHEMA_PATH") == "" {
		...
		if !ok {
			...
			if _, err := os.Stat(schemaPath); err != nil {
				logger.With(ctx, "schema_path", schemaPath, "error", err).Warn("Schema file not found, skipping OPENAPI_SCHEMA_PATH setup")
			} else {
				_ = os.Setenv("OPENAPI_SCHEMA_PATH", schemaPath)
				logger.With(ctx, "schema_path", schemaPath).Info("Set OPENAPI_SCHEMA_PATH for integration tests")
			}
		}
	}
