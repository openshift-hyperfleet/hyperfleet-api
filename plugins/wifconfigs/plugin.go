...
const (
	...
	pluralWifConfigs    = "wifconfigs"
	specSchemaWifConfig = "WifConfigSpec"
)

func init() {
	registry.Register(registry.EntityDescriptor{
		Kind:                   kindWifConfig,
		Plural:                 pluralWifConfigs,
		SpecSchemaName:         specSchemaWifConfig,
		SearchDisallowedFields: []string{"spec"},
	})

	server.RegisterRoutes(pluralWifConfigs, func(apiV1Router *mux.Router, svc server.ServicesInterface) {
		envServices := svc.(*environments.Services)
		descriptor := registry.MustGet(kindWifConfig)
