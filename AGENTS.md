...
plugins/                      # Plugin registration (init-based)
  ...
  generic/plugin.go
openapi/
  openapi.yaml                # EXTRACTED from hyperfleet-api-spec module (not in git, generated by make generate)
...
- **Spec source of truth**: `hyperfleet-api-spec` Go module; update `go.mod` to change spec versions — `openapi/openapi.yaml` is extracted by `make generate` and not tracked in git
- **TypeSpec** definitions live in the `hyperfleet-api-spec` repository and are consumed via its Go module
