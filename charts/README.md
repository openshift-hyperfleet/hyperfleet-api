...
![Version: 1.3.0](https://img.shields.io/badge/Version-1.3.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.0.0-dev](https://img.shields.io/badge/AppVersion-0.0.0--dev-informational?style=flat-square)
...
| config.server | object | `{"host":"0.0.0.0","hostname":"","jwt":{"configs":[],"enabled":false},"port":8000,"tenant":{"dimensions":[],"enabled":false,"system_header":""},"timeouts":{"read":"5s","write":"30s"},"tls":{"cert_file":"","enabled":false,"key_file":""}}` | HTTP server settings |
...
| config.server.tenant | object | `{"dimensions":[],"enabled":false,"system_header":""}` | Tenant enforcement middleware settings. Identity arrives as trusted headers injected by the gateway (Envoy + Authorino); the API never extracts JWT claims for tenancy. Different deployments declare different dimensions via values only — the chart and config loader stay the same. |
| config.server.tenant.enabled | bool | `false` | Enable tenant enforcement middleware |
| config.server.tenant.system_header | string | `""` | Trusted header identifying system callers that bypass tenant scoping (required when tenant.enabled is true) |
| config.server.tenant.dimensions | list | `[]` | Tenant dimensions. Each maps a trusted gateway-injected header to a tenancy key. When `tenant.enabled: true`, at least one dimension must be present, and at least one dimension must have `required: true`. |
...
| config.logging | object | `{"format":"json","level":"info","masking":{"enabled":true,"fields":["password","secret","token","api_key","access_token","refresh_token","client_secret"],"headers":["Authorization","X-API-Key","Cookie","X-Auth-Token","X-Forwarded-Authorization","X-HyperFleet-Identity"]},"output":"stdout"}` | Logging configuration |
...
| config.health.shutdown_timeout | string | `"20s"` | How long the API server waits for in-flight requests to complete during graceful shutdown. Lives under `health` for historical reasons - it governs the API server's drain budget, not the health server's (see terminationGracePeriodSeconds). |
...
| terminationGracePeriodSeconds | int | `45` | Must be > preStop (5s) + shutdown_timeout (20s, API drain) + 14s overhead (2s metrics + 2s health + 5s OTel + 5s DB close) + buffer for JWT close and scheduling |
...
| database.postgresql.image | string | `"docker.io/library/postgres:14.24"` | PostgreSQL container image |
...
| monitoring.tracing | object | `{"enabled":false,"otlpEndpoint":"","otlpProtocol":"grpc","propagators":"tracecontext,baggage","sampler":"parentbased_traceidratio","samplerArg":"1.0","serviceName":"hyperfleet-api"}` | Distributed tracing configuration (OpenTelemetry). This is the single source of truth for tracing - it's templated into HYPERFLEET_TRACING_ENABLED and OTEL_* environment variables in deployment.yaml, which the app's own config system reads (env vars take precedence over the ConfigMap). |
...
| monitoring.tracing.sampler | string | `"parentbased_traceidratio"` | Sampler type |
| monitoring.tracing.samplerArg | string | `"1.0"` | Sampling rate (`1.0` for dev, `0.01` for production) |
