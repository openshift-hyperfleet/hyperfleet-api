...
| config.tenant | object | `{"dimensions":[],"enabled":false,"system_header":"X-HyperFleet-System"}` | Tenant enforcement. When enabled, a middleware resolves gateway-injected headers into a tenancy map and all resource queries are scoped to it. Dimensions map HTTP headers to tenancy keys and are deployment-specific. |
| config.tenant.enabled | bool | `false` | Enable tenant enforcement |
| config.tenant.system_header | string | `"X-HyperFleet-System"` | Header the gateway sets to "true" for trusted system identities |
| config.tenant.dimensions | list | `[]` | Tenant dimensions mapping gateway headers to tenancy keys. At least one dimension must be required: true when enabled. |
