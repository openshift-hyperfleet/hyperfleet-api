...
func NewServeCommand() *cobra.Command {
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Unable to add environment flags to serve command")
		os.Exit(1)
	}
...
func runServe(cmd *cobra.Command, args []string) {
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Unable to initialize environment")
		os.Exit(1)
	}
	...
	if environments.Environment().Config.Logging.OTel.Enabled {
		...
		logger.With(ctx, "otel_enabled", false).Info("OpenTelemetry disabled")
	}
	...
	if err := healthcheckServer.Stop(); err != nil {
		logger.With(ctx, "error", err).Error("Failed to stop healthcheck server")
	}
	if err := apiServer.Stop(); err != nil {
		logger.With(ctx, "error", err).Error("Failed to stop API server")
	}
	if err := metricsServer.Stop(); err != nil {
		logger.With(ctx, "error", err).Error("Failed to stop metrics server")
	}

	if tp != nil {
		if err := telemetry.Shutdown(context.Background(), tp); err != nil {
			logger.With(ctx, "error", err).Error("Failed to shutdown OpenTelemetry")
		}
	}
...
func initLogger() {
	...
	if err != nil {
		logger.With(ctx, "level", cfg.Level, "error", err).Warn("Invalid log level, using default")
		level = slog.LevelInfo
	}

	format, err := logger.ParseLogFormat(cfg.Format)
	if err != nil {
		logger.With(ctx, "format", cfg.Format, "error", err).Warn("Invalid log format, using default")
		format = logger.FormatJSON
	}

	output, err := logger.ParseLogOutput(cfg.Output)
	if err != nil {
		logger.With(ctx, "output", cfg.Output, "error", err).Warn("Invalid log output, using default")
		output = os.Stdout
	}
