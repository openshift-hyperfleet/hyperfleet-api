...
func (s metricsServer) Start() {
	...
	if env().Config.Metrics.EnableHTTPS {
		...
		logger.With(ctx, "bind_address", env().Config.Metrics.BindAddress).Info("Serving Metrics with TLS")
		err = s.httpServer.ListenAndServeTLS(env().Config.Server.HTTPSCertFile, env().Config.Server.HTTPSKeyFile)
	} else {
		logger.With(ctx, "bind_address", env().Config.Metrics.BindAddress).Info("Serving Metrics without TLS")
