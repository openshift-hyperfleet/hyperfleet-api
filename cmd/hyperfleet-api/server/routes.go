...
func registerApiMiddleware(router *mux.Router) {
	...
	if err != nil {
		...
		logger.With(ctx, "schema_path", schemaPath, "error", err.Error()).Warn("Failed to load schema validator")
		...
		logger.With(ctx, "schema_path", schemaPath).Info("Schema validation enabled")
