...
func (s apiServer) Serve(listener net.Listener) {
	...
	if env().Config.Server.EnableHTTPS {
		...
		// Serve with TLS
		logger.With(ctx, "bind_address", env().Config.Server.BindAddress).Info("Serving with TLS")
		err = s.httpServer.ServeTLS(listener, env().Config.Server.HTTPSCertFile, env().Config.Server.HTTPSKeyFile)
	} else {
		logger.With(ctx, "bind_address", env().Config.Server.BindAddress).Info("Serving without TLS")
...
func (s apiServer) Start() {
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Unable to start API server")
		os.Exit(1)
	}
	s.Serve(listener)
	...
	if err := env().Database.SessionFactory.Close(); err != nil {
		logger.With(ctx, "error", err).Error("Error closing database connection")
	}
}
