...
func (s healthCheckServer) Start() {
	...
	if env().Config.HealthCheck.EnableHTTPS {
		...
		// Serve with TLS
		logger.With(ctx, "bind_address", env().Config.HealthCheck.BindAddress).Info("Serving HealthCheck with TLS")
		err = s.httpServer.ListenAndServeTLS(env().Config.Server.HTTPSCertFile, env().Config.Server.HTTPSKeyFile)
	} else {
		logger.With(ctx, "bind_address", env().Config.HealthCheck.BindAddress).Info("Serving HealthCheck without TLS")
