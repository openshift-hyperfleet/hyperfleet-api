...
func (e *Env) Initialize() error {
	...
	logger.With(ctx, "environment", e.Name).Info("Initializing environment")

	envImpl, found := environments[e.Name]
	if !found {
		logger.With(ctx, "environment", e.Name).Error("Unknown runtime environment")
		os.Exit(1)
	}

	if err := envImpl.OverrideConfig(e.Config); err != nil {
		logger.With(ctx, "error", err).Error("Failed to configure ApplicationConfig")
		os.Exit(1)
	}

	messages := environment.Config.ReadFiles()
	if len(messages) != 0 {
		logger.With(ctx, "errors", strings.Join(messages, "\n")).Error("Unable to read configuration files")
		os.Exit(1)
	}
	...
	if err := envImpl.OverrideDatabase(&e.Database); err != nil {
		logger.With(ctx, "error", err).Error("Failed to configure Database")
		os.Exit(1)
	}
	...
	if err := envImpl.OverrideClients(&e.Clients); err != nil {
		logger.With(ctx, "error", err).Error("Failed to configure Clients")
		os.Exit(1)
	}

	e.LoadServices()
	if err := envImpl.OverrideServices(&e.Services); err != nil {
		logger.With(ctx, "error", err).Error("Failed to configure Services")
		os.Exit(1)
	}
	...
	if err := envImpl.OverrideHandlers(&e.Handlers); err != nil {
		logger.With(ctx, "error", err).Error("Failed to configure Handlers")
		os.Exit(1)
	}
...
func (e *Env) LoadClients() error {
	...
	if err != nil {
		logger.With(ctx, "error", err).Error("Unable to create OCM Authz client")
		return err
	}
...
func (e *Env) Teardown() {
	...
	if e.Database.SessionFactory != nil {
		if err := e.Database.SessionFactory.Close(); err != nil {
			logger.With(ctx, "error", err).Error("Error closing database session factory")
		}
	}
	if e.Clients.OCM != nil {
		if err := e.Clients.OCM.Close(); err != nil {
			logger.With(ctx, "error", err).Error("Error closing OCM client")
		}
	}
}
...
func setConfigDefaults(flags *pflag.FlagSet, defaults map[string]string) error {
	...
	for name, value := range defaults {
		if err := flags.Set(name, value); err != nil {
			logger.With(ctx, "flag", name, "error", err).Error("Error setting flag")
			return err
		}
	}
	return nil
