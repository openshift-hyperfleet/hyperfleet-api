...
- OpenAPI source spec: embedded in `github.com/openshift-hyperfleet/hyperfleet-api-spec/schemas` Go module; `openapi/openapi.yaml` is extracted by `make generate` (not tracked in git)
...
Two `openapi.yaml` files exist (both generated, neither tracked in git):
- `openapi/openapi.yaml` — extracted from spec module by `make generate` (61KB, has `$ref`)
- `pkg/api/openapi/api/openapi.yaml` — generated by oapi-codegen (fully resolved, embedded in binary)
...
- **Spec source of truth**: `hyperfleet-api-spec` Go module; `openapi/openapi.yaml` is extracted during `make generate` and is not tracked in git — update `go.mod` to change spec versions
