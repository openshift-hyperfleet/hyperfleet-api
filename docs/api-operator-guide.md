...
The API validates resource `spec` fields (e.g., cluster, nodepool) against an OpenAPI schema. The schema is embedded in the binary from the `github.com/openshift-hyperfleet/hyperfleet-api-spec` Go module — no configuration is required. Schema validation is always active.
...
| **400 Bad Request** | Resource spec doesn't match OpenAPI schema | The schema is embedded in the binary from the `hyperfleet-api-spec` Go module. Check API logs for validation error details: `kubectl logs -n hyperfleet-system deployment/hyperfleet-api \| grep "Invalid"`. Fix the spec fields in your request payload. |
